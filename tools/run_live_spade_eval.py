...
2. ProofPack V0-V4 Qualification Ladder certifies environment validity
3. Reasoning Agent (LLM) plays the environment (Unhinted vs Hinted)
...
sys.path.insert(0, "/Users/sergio.soage/code/proofpack-spade/packages/env/src")
sys.path.insert(0, "/Users/sergio.soage/code/assay-spade")
...
Requirements:
1. Class name must end in `Env` (e.g. `PuzzleEnv`).
2. Implement `__init__(self, max_turns=10, **kwargs)`
3. Implement `reset(self, seed=None)` returning `(observation: str, info: dict)`. Format instructions should ask for `\\boxed{{answer}}`.
4. Implement `solution(self)` returning the string answer (or list of turn-by-turn answers).
5. Implement `step(self, action: str)` returning `(observation: str, reward: float, terminated: bool, truncated: bool, info: dict)`.
6. Reward must be 1.0 when correct and 0.0 otherwise.
...
AGENT_PROMPT = """You are a reasoning agent solving a puzzle environment.
Observation: {obs}
{hint_text}

Analyze the problem step-by-step and provide your final answer clearly formatted as \\boxed{{answer}}."""
...
async def run_live_eval(args):
    ...
    for idx, line in enumerate(env_code.splitlines()[:35], 1):
        print(f"   {idx:02d} | {line}")
    if len(env_code.splitlines()) > 35:
        print(f"   ... ({len(env_code.splitlines()) - 35} more lines)")
    ...
    print("-" * 76)
    report = qualify_spade_environment(env_code, seeds=[0, 1, 42])
    ...
    if not report.passed:
        print("\n❌ Environment failed ProofPack qualification. Halting fail-closed.")
        return
    ...
    # -------------------------------------------------------------
    print("\n" + "-" * 76)
    print("🎮 [STEP 4/5] REASONING AGENT DUAL ROLLOUTS (UNHINTED vs HINTED)")
    print("-" * 76)
    target = SpadeEnvironmentTarget(env_code)

    # 4A. Inspect Problem Instance and Oracle
    test_env = target.instantiate()
    obs_test, _ = test_env.reset(seed=7)
    oracle_sol = test_env.solution()
    print(f"📋 Problem Observation (Seed 7):\n   {obs_test.strip()}\n")
    print(f"🔑 True Oracle Solution from Environment: {oracle_sol}\n")

    # 4B. Unhinted rollout
    print("▶️ [Lane 1: Unhinted Play]")
    env_unhinted = target.instantiate()
    obs_unhinted, _ = env_unhinted.reset(seed=7)
    agent_response_unhinted = await call_llm(
        client_or_bin, model, AGENT_PROMPT.format(obs=obs_unhinted, hint_text=""),
        provider=args.provider
    )
    step_res_unhinted = env_unhinted.step(agent_response_unhinted)
    reward_unhinted = float(step_res_unhinted[1])
    print(f"   Agent Reasoning Summary: {agent_response_unhinted.splitlines()[-1] if agent_response_unhinted.splitlines() else agent_response_unhinted}")
    print(f"   Environment Feedback:     {step_res_unhinted[0]}")
    print(f"   Return r_A(e):            {reward_unhinted:.2f}\n")

    # 4C. Hinted rollout
    print("▶️ [Lane 2: Hinted Play]")
    env_hinted = target.instantiate()
    obs_hinted, _ = env_hinted.reset(seed=7)
    agent_response_hinted = await call_llm(
        client_or_bin, model, AGENT_PROMPT.format(obs=obs_hinted, hint_text=f"\nPrivileged Hint: {hint_text}"),
        provider=args.provider
    )
    step_res_hinted = env_hinted.step(agent_response_hinted)
    reward_hinted = float(step_res_hinted[1])
    print(f"   Agent Reasoning Summary: {agent_response_hinted.splitlines()[-1] if agent_response_hinted.splitlines() else agent_response_hinted}")
    print(f"   Environment Feedback:     {step_res_hinted[0]}")
    ...
    # -------------------------------------------------------------
    # Step 5: Assay Benchmark & Statistics Ingestion
    # -------------------------------------------------------------
    print("\n" + "-" * 76)
    print("📊 [STEP 5/5] ASSAY CURRICULUM EMISSION & CLUSTERED STATISTICAL INGESTION")
    print("-" * 76)
    task_payload = SpadeTaskPayload(
        task_id="live-task-detailed-001",
        environment_name=report.environment_name,
        skill=args.skill,
        code=env_code,
        solution=str(oracle_sol),
    )
    manifest = emit_spade_curriculum([task_payload], curriculum_id="live-detailed-manifest")
    print(f"   Assay Manifest Digest: {manifest.content_digest}")
    print(f"   Vendi Diversity Score: {manifest.vendi_diversity_score:.4f}")

    cluster = SpadeClusterData(
        cluster_id="live-task-detailed-001",
        candidate_returns=(reward_hinted,),
        base_returns=(reward_unhinted,),
        hinted_returns=(reward_hinted,),
        regret=regret,
    )
    cert_report = certify_spade_curriculum_gain(
        candidate_arm=f"{model}-hinted",
        base_arm=f"{model}-unhinted",
        clusters=[cluster],
    )
    print(f"   Assay Promotion Gating: {cert_report.rationale}")

    print("\n" + "=" * 76)
    print("🏁 VERIFICATION CONFIRMED END-TO-END")
    print("=" * 76 + "\n")
