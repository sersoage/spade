...
if [ ! -d "${PROJECT_ROOT}/spade/external/rlve/Gym" ]; then
   echo "ERROR: vendored RLVE not found at spade/external/rlve/Gym." >&2
   exit 1
fi
...
SPADE_ARGS=(
   ...
   --spade-fixed-env-source "${RLVE_SOURCES[@]}"
   --spade-game-regeneration-interval 1
   --spade-fixed-env-same-problem
   --spade-trajectories-per-game 16
   --spade-difficulty-variant "${DIFFICULTY_VARIANT}"
   --spade-difficulty-tau-acc 0.9
   --spade-difficulty-tau-num 128
   --spade-difficulty-d-delta 4
   --spade-lp-gamma-fast 0.35
   --spade-lp-gamma-slow 0.15
   --spade-actor-template qwen3_game
   --spade-actor-temperature 1.0
