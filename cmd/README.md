...
└── tinker/      # Tinker training backend (alternative)
```

Only the commands used in the paper are included.
...
are not bundled. The Tinker launchers (`cmd/tinker/*/train_spade_tinker.sh`)
...
`spade/core/eval/bfcl_evaluator.py` defaults to
`/workspace/spade-workspace/bfcl/gorilla/berkeley-function-call-leaderboard`
and honours `BFCL_REPO_PATH`.

### `CORPUS_FILE` schema

The adaptive games and tool-use recipes require `CORPUS_FILE`, a JSONL
grounding corpus consumed by `spade/core/corpus.py`. The paper corpora are not
...
`--spade-corpus-max-doc-tokens` whitespace-separated words (6000 for games,
4000 for tool use) and sampled uniformly with `--spade-corpus-seed`.
...
These launchers encode the paper recipes; they do not claim
