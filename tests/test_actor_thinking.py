...
import spade.core.orchestrator as orchestrator_module
...
from spade.slime.trajectory_converter import (
...
def make_orchestrator(adapter: FakeAdapter, actor_enable_thinking: Optional[bool]) -> SpadeOrchestrator:
    config = SpadeConfig(
...
def test_e_truncated_think_no_closing_tag():
    ...
    for k, (record, sample) in enumerate(zip(records, fanned)):
        ...
        # spade_compact_filter / truncation metrics see it.
...
def test_f_compact_filtered_episode_zeroes_turn_masks():
    ...
    # Simulate spade_compact_filter zeroing the truncated episode.
