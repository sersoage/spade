#!/usr/bin/env python3
"""
SPADE training script using Tinker framework.

This is the main entry point for SPADE training with the Tinker backend.
Uses SpadeOrchestrator for unified dual-role training.
"""
...
from spade.tinker.rollout import spade_generate_rollout
from spade.tinker.train_step import train_step
from spade.tinker.evaluators import create_aime_evaluators
...
@chz.chz
class SpadeConfig(train.Config):
    """Configuration for SPADE training with Tinker."""
...
async def do_spade_training_loop(
    start_batch: int,
    end_batch: int,
    num_batches: int,
    cfg: SpadeConfig,
    ...
    """
    Main SPADE training loop using SpadeOrchestrator.
    ...
    for i_batch in range(start_batch, end_batch):
        ...
        traj_groups, rollout_metrics = await spade_generate_rollout(
...
async def create_spade_train_loop(cfg: SpadeConfig):
    """
    Create and run SPADE training loop.

    This is the main entry point that sets up all components and runs training.
    """
    if not cfg.log_path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        cfg = chz.replace(cfg, log_path=f"./experiments/spade_tinker/{timestamp}")
    os.makedirs(cfg.log_path, exist_ok=True)

    if cfg.wandb_name is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        cfg = chz.replace(cfg, wandb_name=f"spade_tinker_{timestamp}")

    logger.info("=" * 80)
    logger.info("SPADE Training Configuration (Tinker Backend)")
    logger.info("=" * 80)
...
def main():
    """Main entry point for SPADE Tinker training."""
    cli_config = chz.entrypoint(SpadeConfig)

    cli_utils.check_log_dir(cli_config.log_path, behavior_if_exists="ask")

    asyncio.run(create_spade_train_loop(cli_config))
