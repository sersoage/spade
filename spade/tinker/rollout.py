"""Thin wrapper for SPADE rollouts using Tinker backend.

This module provides a simple interface to SpadeOrchestrator for Tinker training.
All core logic lives in spade/core/orchestrator.py.
"""
...
from spade.tinker.model_adapter import TinkerModelAdapter
from spade.tinker.trajectory_converter import spade_trajectory_to_tinker_trajectory

if TYPE_CHECKING:
    from train_spade_tinker import SpadeConfig as TinkerSpadeConfig
...
_ORCHESTRATOR: Optional[SpadeOrchestrator] = None


def get_learning_potentials(cfg: "TinkerSpadeConfig") -> Dict[str, LearningPotential]:
...
def get_orchestrator(
    ...
    tokenizer,
    renderer,
    cfg: "TinkerSpadeConfig",
) -> SpadeOrchestrator:
    """Get or create the SpadeOrchestrator (reuses across rollouts)."""
    ...
    if _ORCHESTRATOR is None:
        ...
        spade_config = SpadeConfig(
            ...
            use_proofpack_qualification=cfg.use_proofpack_qualification,
            proofpack_seeds=list(cfg.proofpack_seeds),
            proofpack_timeout_seconds=cfg.proofpack_timeout_seconds,
        ...
        _ORCHESTRATOR = SpadeOrchestrator(
            model=model_adapter,
            config=spade_config,
...
async def spade_generate_rollout(
    sampling_client: SamplingClient,
    current_step: int,
    cfg: "TinkerSpadeConfig",
    renderer,
    ...
    """Perform SPADE rollout and return list of Tinker TrajectoryGroups with metrics.
    ...
    for traj in env_trajectories:
        tinker_traj = spade_trajectory_to_tinker_trajectory(traj, role="environment")
    ...
    for traj in actor_trajectories:
        tinker_traj = spade_trajectory_to_tinker_trajectory(traj, role="actor")
        trajectory_groups.append(TrajectoryGroup(
            trajectories_G=[tinker_traj],
            final_rewards_G=[traj.reward or 0.0],
