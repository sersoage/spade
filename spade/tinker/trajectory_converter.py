"""Converter between SpadeOrchestrator's episode Trajectory and Tinker's Transition/Trajectory format.
...
from spade.core.types import Trajectory as SpadeTrajectory

logger = logging.getLogger(__name__)


def spade_trajectory_to_tinker_trajectory(
    trajectory: SpadeTrajectory,
    role: str = "actor",
) -> TinkerTrajectory:
    """Convert a SPADE episode Trajectory to a Tinker Trajectory.
...
def _env_trajectory_to_tinker(trajectory: SpadeTrajectory) -> TinkerTrajectory:
...
def _actor_trajectory_to_tinker(trajectory: SpadeTrajectory) -> TinkerTrajectory:
...
def spade_trajectory_to_tinker_trajectories(
    trajectory: SpadeTrajectory,
    ...
    tinker_traj = spade_trajectory_to_tinker_trajectory(trajectory, role=role)
    return [tinker_traj]
