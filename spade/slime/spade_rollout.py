"""SPADE rollout function for Slime backend using SpadeOrchestrator.

This module provides a custom rollout function that integrates SpadeOrchestrator
...
from spade.core.corpus import CorpusLoader
from spade.core.corpus_orchestrator import CorpusGroundedOrchestrator
from spade.core.env_memory import EnvironmentMemory
from spade.core.eval.fixed_model_eval import run_fixed_model_evaluation
...
from spade.core.prompts.tool_use_template import TOOL_USE_SKILLS
...
def _maybe_cap_rollout_memory() -> None:
    """Cap this process's address space via SPADE_ROLLOUT_RLIMIT_GB (0/unset = off).
    ...
    """
    gb = int(os.environ.get("SPADE_ROLLOUT_RLIMIT_GB", "0") or 0)
    if gb <= 0:
        return
    cap = gb * 1024**3
...
def _get_or_create_corpus(args: Namespace):
    ...
    if _CORPUS is None:
        _CORPUS = CorpusLoader(
            corpus_path=args.spade_corpus_file,
            max_doc_tokens=args.spade_corpus_max_doc_tokens,
            seed=args.spade_corpus_seed,
        )
        num_loaded = _CORPUS.load()
        if num_loaded == 0:
            raise RuntimeError(
                f"[SPADE] Corpus file contains no usable documents: "
                f"{args.spade_corpus_file}"
            )
        logger.info(
            f"[SPADE] Corpus loaded: {num_loaded} documents "
            f"from {args.spade_corpus_file}"
        )
    return _CORPUS


def _define_spade_wandb_metrics():
    """Register SPADE metrics on the appropriate WandB step axes."""
    ...
    try:
        ...
        if wandb.run is not None:
            ...
            logger.info("[SPADE] Registered gem_eval/* (eval/step) + catch-all * (rollout/step)")
    except Exception as e:
        logger.warning("[SPADE] Failed to define wandb metrics: %s", e)
...
def get_configured_skills(args: Namespace) -> List[str]:
    ...
    Respects --spade-game-type and validates skills against the correct skill set.
    If --spade-skills is not specified, uses default skills for the game type.
    """
    game_type = getattr(args, "spade_game_type", "cognitive")
    defaults, available = _GAME_TYPE_SKILLS.get(game_type, _GAME_TYPE_SKILLS["cognitive"])

    if args.spade_skills:
        # Accept underscore aliases that survive shell and Ray argument forwarding.
        requested = [s.replace("_", " ").strip() for s in args.spade_skills]
...
def _inert_copy(template: Sample, index: int) -> Sample:
    ...
    global_batch_size groups) but contributes no gradient. Tagged spade_is_pad so the
    ...
    dup.index = index
    dup.metadata = {**(dup.metadata or {}), "spade_is_pad": True}
    return dup


def spade_generate_rollout(
    args: Namespace,
    rollout_id: int,
    data_source: Callable,
    evaluation: bool = False,
) -> RolloutFnTrainOutput:
    """SPADE rollout function using SpadeOrchestrator for dual-role training.

    This function replaces Slime's standard rollout with SPADE's dual-role approach:
    ...
    Args:
        ...
        data_source: Data source callable for fetching prompts (not used in SPADE)
    ...
    _define_spade_wandb_metrics()

    # Route to fixed-env mode if configured
    if args.spade_mode == "fixed_env":
        from spade.slime.fixed_env_rollout import spade_fixed_env_rollout
        return spade_fixed_env_rollout(args, rollout_id, data_source, evaluation)
    ...
    del data_source  # data_source is not used in SPADE, but Slime requires it to be provided.

    # Validate delayed proposer training config
    if args.spade_proposer_training_delay > 0 and not args.use_tis:
        logger.warning(
            "[SPADE] proposer-training-delay > 0 but --use-tis not enabled. "
            "IS correction recommended for delayed training."
        )
    if args.spade_env_reward_variant in ("micro_lp", "blend") and args.spade_proposer_training_delay == 0:
        raise ValueError(
            f"[SPADE] {args.spade_env_reward_variant} env reward variant requires --spade-proposer-training-delay > 0."
        )

    # Thinking fan-out varies sample counts, so SPADE computes group advantages.
    if args.spade_actor_enable_thinking and args.rewards_normalization:
        raise ValueError(
            "[SPADE] --spade-actor-enable-thinking requires "
    ...
    spade_config = SpadeConfig(
        ...
        env_generation_template=args.spade_env_generation_template,
        env_max_tokens=args.spade_env_max_tokens,
        env_enable_thinking=True if args.spade_env_enable_thinking else None,
        # Proposer-only thinking explicitly keeps the actor non-thinking.
        actor_enable_thinking=(
            True if args.spade_actor_enable_thinking
            else (False if args.spade_env_enable_thinking else None)
        ),
        env_repair_turns=args.spade_env_repair_turns,
        persist_rejected=args.spade_persist_rejected,
        ...
        actor_template=args.spade_actor_template,
        actor_max_tokens=args.spade_actor_max_tokens,
        max_context_length=args.spade_max_context_length,
        use_format_reward=args.spade_use_format_reward,
        format_reward_value=args.spade_format_reward_value,
        use_solver_variance_reward=args.spade_use_solver_variance_reward,
        # Self-judge parameters
        use_self_judge=args.spade_use_self_judge,
        self_judge_temperature=args.spade_self_judge_temperature,
        self_judge_max_tokens=args.spade_self_judge_max_tokens,
        self_judge_penalty=args.spade_self_judge_penalty,
        self_judge_max_turns_to_show=args.spade_self_judge_max_turns_to_show,
        # Environment reward scaling parameters
        env_reward_scaling_variant=args.spade_env_reward_scaling_variant,
        max_env_reward_scale=args.spade_max_env_reward_scale,
        auto_compute_env_reward_scale=args.spade_auto_compute_env_reward_scale,
        # Training control
        train_on_env_trajectories=args.spade_train_on_env_trajectories,
        ...
        env_reward_variant=args.spade_env_reward_variant,
        hint_mode=args.spade_hint_mode,
        hint_model=args.spade_hint_model,
        hint_api_key_env=args.spade_hint_api_key_env,
        hint_api_base_url=args.spade_hint_api_base_url,
        hint_temperature=args.spade_hint_temperature,
        hint_max_tokens=args.spade_hint_max_tokens,
        hint_plays_per_game=args.spade_hint_plays_per_game,
        # Fixed model evaluation parameters
        fixed_eval_model=args.spade_fixed_eval_model,
        fixed_eval_plays_per_game=args.spade_trajectories_per_game,
        fixed_eval_max_concurrent=args.spade_fixed_eval_max_concurrent,
        fixed_eval_temperature=args.spade_fixed_eval_temperature,
        fixed_eval_max_tokens=args.spade_fixed_eval_max_tokens,
        fixed_eval_api_base_url=args.spade_fixed_eval_api_base_url,
        fixed_eval_api_key_env=args.spade_fixed_eval_api_key_env,
        # Environment validator parameters
        use_env_validator=args.spade_use_env_validator,
        env_validator_model=args.spade_env_validator_model,
        env_validator_api_key_env=args.spade_env_validator_api_key_env,
        env_validator_api_base_url=args.spade_env_validator_api_base_url,
        env_validator_temperature=args.spade_env_validator_temperature,
        env_validator_max_tokens=args.spade_env_validator_max_tokens,
        # Delayed LP: skip per-rollout LP updates when delay > 0 with LP variant
        skip_lp_update=(
            args.spade_proposer_training_delay > 0
            and args.spade_env_reward_variant == "learning_potential"
        ),
        # Corpus-grounded generation
        corpus_file=args.spade_corpus_file,
        corpus_max_doc_tokens=args.spade_corpus_max_doc_tokens,
        corpus_seed=args.spade_corpus_seed,
        # Action format
        action_format=args.spade_action_format,
        # Game type for self-play generation
        game_type=args.spade_game_type,
    )
    ...
    env_memory = None
    if args.spade_use_env_memory:
        global _ENV_MEMORY
        if _ENV_MEMORY is None:
            _ENV_MEMORY = EnvironmentMemory(max_size=args.spade_env_memory_max_size)
            memory_path = Path(args.spade_cache_dir) / "env_memory.json"
            _ENV_MEMORY.load(memory_path)
        env_memory = _ENV_MEMORY

    # Create orchestrator (corpus-grounded if corpus file provided)
    if args.spade_corpus_file:
    ...
    game_regeneration_interval = args.spade_game_regeneration_interval
    should_regenerate = (
    ...
    cache_dir = args.spade_cache_dir
    if cache_dir:
        cache_dir = Path(cache_dir)

    skills = get_configured_skills(args)
    skills = select_active_skills(
        skills, rollout_id, game_regeneration_interval, args.spade_skills_per_regen
    )
    if args.spade_skills_per_regen > 0:
        logger.info("[SPADE] rollout %d active skills (round-robin %d/%d): %s",
                    rollout_id, args.spade_skills_per_regen,
                    len(get_configured_skills(args)), skills)
    num_games = args.rollout_batch_size
    difficulty = args.spade_game_difficulty
    use_solver_variance_reward = args.spade_use_solver_variance_reward
    # num_games * trajectories_per_game = global_batch_size
    global_batch_size = args.global_batch_size
    trajectories_per_game = max(1, global_batch_size // num_games)

    logger.info(
        f"[SPADE] Batch calculation: global_batch_size={global_batch_size}, "
    ...
    delay = args.spade_proposer_training_delay
    ...
    if delay > 0:
        ...
        if should_regenerate and args.spade_env_reward_variant in ("regret_based", "blend"):
    ...
    if delay > 0 and env_samples_for_training:
        ...
        if accumulated:
            env_reward_scale = compute_env_reward_scale(
                ...
                regeneration_interval=game_regeneration_interval,
                max_scale=args.spade_max_env_reward_scale,
            ) if args.spade_auto_compute_env_reward_scale else 1.0

            if args.spade_env_reward_variant == "blend":
                ...
                recompute_metrics = recompute_delayed_env_rewards_blend(
                    ...
                    regret_weight=args.spade_regret_weight,
                    micro_lp_weight=args.spade_micro_lp_weight,
                    regret_scale=args.spade_regret_scale,
                    micro_lp_scale=args.spade_micro_lp_scale,
                    micro_lp_signed=not args.spade_micro_lp_unsigned,
                    micro_lp_slope=(args.spade_micro_lp_estimator == "slope"),
                    frontier_weight=args.spade_frontier_weight,
                    frontier_scale=args.spade_frontier_scale,
                    plateau_weight=args.spade_plateau_weight,
                    plateau_lo=args.spade_plateau_lo,
                    plateau_hi=args.spade_plateau_hi,
                    plateau_ramp=args.spade_plateau_ramp,
                    regret_floor=args.spade_regret_floor,
                )
            elif args.spade_env_reward_variant == "micro_lp":
                recompute_metrics = recompute_delayed_env_rewards_micro_lp(
                    env_samples=env_samples_for_training,
                    accumulated_game_rewards=accumulated,
                    env_reward_scale=env_reward_scale,
                )
            elif args.spade_env_reward_variant == "regret_based":
                ...
                recompute_metrics = recompute_delayed_env_rewards(
                    ...
                    env_reward_scale=env_reward_scale,
                    use_solver_variance_reward=args.spade_use_solver_variance_reward,
                    update_lp=(args.spade_env_reward_variant == "learning_potential"),
                )

            total_plays = sum(len(r) for sd in accumulated.values() for r in sd.values())
            logger.info(
                f"[SPADE] Recomputed delayed env rewards "
    ...
    if not all_samples:
        ...
        if fallback is None:
            raise ValueError(
                f"[SPADE] Rollout {rollout_id}: 0 samples and no fallback (first rollout, "
    ...
    if 0 < len(all_samples) < global_batch_size:
        ...
        for i, sample in enumerate(all_samples):
            sample.index = i
        logger.warning(
            f"[SPADE] Rollout {rollout_id}: partial batch ({n_real} < {global_batch_size}); "
    ...
    if len(grouped_samples) > global_batch_size:
        ...
        logger.warning(
            f"[SPADE] Trimmed {n_trim} surplus (now actor) samples to meet global_batch_size"
        )
    ...
    if 0 < n_real < global_batch_size:
        if _scale_supported:
            ...
            metrics["rollout/n_real_samples"] = float(n_real)
            logger.warning(
                f"[SPADE] Rollout {rollout_id}: partial batch "
    ...
    if args.spade_actor_enable_thinking:
        n_turn_samples = 0
        n_fanned_episodes = 0
        fanned_groups: List[List[Sample]] = []
        for grp in grouped_samples:
            episode = grp[0]
            is_pad = bool((episode.metadata or {}).get("spade_is_pad"))
    ...
    fixed_eval_interval = args.spade_fixed_eval_interval

    if fixed_eval_interval > 0:
        # Validate API key is available upfront — fail loud, not silent
        _api_key_env = spade_config.fixed_eval_api_key_env
        if not os.environ.get(_api_key_env):
            raise RuntimeError(
                f"[SPADE] Fixed model evaluation is enabled (interval={fixed_eval_interval}) "
                f"but {_api_key_env} environment variable is not set. "
                f"Either set the API key or disable fixed eval (--spade-fixed-eval-interval 0)."
            )

        if rollout_id % fixed_eval_interval == 0:
            try:
                fixed_eval_metrics = _run_fixed_model_evaluation(spade_config, games_dir)
                metrics.update(fixed_eval_metrics)
                logger.info(
                    "[SPADE] Fixed model evaluation complete: pass_rate=%.2f, difficulty_score=%.2f",
                    fixed_eval_metrics.get("fixed_eval/overall_pass_rate", 0.0),
                    fixed_eval_metrics.get("fixed_eval/difficulty_score", 0.0),
                )
            except Exception as e:
                logger.error("[SPADE] Fixed model evaluation failed: %s", e)
                metrics["fixed_eval/error"] = 1.0

    return RolloutFnTrainOutput(samples=grouped_samples, metrics=metrics)


def _run_fixed_model_evaluation(config: SpadeConfig, games_dir: Path) -> Dict[str, float]:
    """Run fixed model evaluation and return metrics dict.

    Args:
        config: SpadeConfig with evaluation parameters
