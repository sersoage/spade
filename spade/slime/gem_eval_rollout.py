...
--spade-gem-eval-config). If a section is absent, that branch silently skips.
"""
...
from spade.core.eval.gem_evaluator import GemEvaluator, GemEvalResult
from spade.core.eval.gem_tasks import load_gem_eval_config
from spade.core.eval.tau2_evaluator import Tau2Evaluator, Tau2EvalResult
from spade.core.eval.tau2_tasks import load_tau2_eval_config
from spade.core.eval.bfcl_evaluator import BfclEvaluator, BfclEvalResult, load_bfcl_eval_config
...
def _run_gem_branch(
    ...
    if not args.spade_gem_eval_config:
        return {}

    defaults, task_specs = load_gem_eval_config(args.spade_gem_eval_config)
    if not task_specs:
...
def _run_tau2_branch(
    ...
    try:
        defaults, task_specs = load_tau2_eval_config(args.spade_gem_eval_config)
    except Exception as exc:
        logger.error("[TAU2-EVAL] Failed to load tau2 eval config: %s", exc)
        return {}

    if not task_specs:
        # No tau2_eval section or empty tasks list; skip.
        return {}
...
def _run_bfcl_branch(
    ...
    bfcl_config = load_bfcl_eval_config(args.spade_gem_eval_config)
    if not bfcl_config:
...
def spade_gem_eval_rollout(
    args: Namespace,
    rollout_id: int,
    data_source: Callable,
) -> RolloutFnEvalOutput:
    """Run the SPADE eval rollout (GEM + tau2 + Slime built-in).
    ...
    if not metrics and not slime_data:
        logger.warning(
            ...
            "--spade-gem-eval-config points to a file with gem_eval or "
