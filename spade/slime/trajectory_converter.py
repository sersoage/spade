"""Converter between SpadeOrchestrator's Trajectory and Slime's Sample format.
...
def trajectory_to_slime_sample(
    ...
    """
    # Map SPADE TrajectoryStatus to Slime Sample.Status
    status_map = {
...
def fan_out_thinking_sample(sample: Sample, tokenizer) -> List[Sample]:
    ...
    defaults, both required by SPADE's precomputed reward pipeline):
    - Reward: the FULL episode reward is broadcast to every turn sample
      instead of the uniform reward/K split. SPADE precomputes the GRPO
    ...
    - Status: the episode status (e.g. TRUNCATED) is restored on every
      ...
      which would defeat spade_compact_filter and truncation metrics.
...
def trajectories_to_grouped_samples(
    ...
    For SPADE, we treat each sample as its own group.
