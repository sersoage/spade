...
from spade.core.utils.env_rewards import _mean_or_zero
...
# -----------------------------------------------------------------------------
# Variants, selected at runtime by --spade-env-reward-variant
# -----------------------------------------------------------------------------
...
def recompute_delayed_env_rewards(
    ...
    skipped (via skip_lp_update on SpadeConfig) so this is the only place
    LP gets updated.
...
def recompute_delayed_env_rewards_blend(
    ...
    - regret (matched timing): R_hint - R_no_hint, BOTH measured at the regen step
      ...
      games). Match the play counts via --spade-hint-plays-per-game == trajectories
