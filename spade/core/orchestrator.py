"""SpadeOrchestrator: Unified dual-role orchestration for SPADE training.
...
from spade.core.types import (
    Trajectory,
    TrajectoryStatus,
    SpadeConfig,
)
from spade.core.model_adapter import ModelAdapter
from spade.core.learning_potential import LearningPotential, GameBaselineTracker
from spade.core.game_policy import GamePolicy
from spade.core.envs.synthetic_game_env import make_synthetic_env, criteria_throw_at_reset
from spade.core.prompts import (
...
from spade.core.prompts.tool_use_template import (
    TOOL_USE_SKILLS,
...
from spade.core.utils.tool_call_parser import parse_tool_calls
from spade.core.batched import (
...
class SpadeOrchestrator:
    """Orchestrates dual-role SPADE training with unified logic for all backends.
    ...
    async def _play_game_thinking_async(
        ...
        - Each generation is captured as a raw turn record
          ...
          as plain dicts because spade/core must not import slime).
        ...
        try:
            for turn in range(self.config.max_turns):
                ...
                if not parsed["calls"]:
                    ...
                    _malformed_tc = (
                        os.getenv("SPADE_MASK_MALFORMED_TOOLCALL") == "1"
    ...
    async def play_games_async(
        ...
        async def run_game_group(game_file: Path, skill: str, group_index: int) -> List:
            ...
            each hung game to one stranded thread. Set SPADE_PLAY_CANARY=0 to
            disable the gate.
            """
        ...
        # SPADE_PLAY_CANARY=0 restores the flat all-at-once launch.
        use_canary = os.environ.get("SPADE_PLAY_CANARY", "1") != "0"
        tasks = []
    ...
    async def self_judge_game_async(
        ...
        try:
            ...
            from spade.core.utils import extract_boxed_answer
