...
#
# Use via os.environ["SPADE_MULTITURN_ENV_GEN"] = "1"; orchestrators
...
def get_env_gen_prompt_fn():
    """Return the env-gen prompt function based on SPADE_MULTITURN_ENV_GEN env var.

    Default: generate_tool_use_prompt (single-task structure, original).
    SPADE_MULTITURN_ENV_GEN=1: generate_tool_use_prompt_multiturn (BFCL-style
        multi-turn structure with 3-5 atomic user messages revealed progressively).
    """
    import os
    if os.environ.get("SPADE_MULTITURN_ENV_GEN") == "1":
